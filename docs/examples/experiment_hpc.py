...
if __name__ == "__main__":
    ...
    for signal_gain in range(1, 11):
        ###############################
        ## Build and run experiment ###
        ###############################

        mr.config.retina_parameters.signal_gain = float(signal_gain)
        contrast = str(mr.config.visual_stimulus_parameters.contrast).replace(".", "p")
        sf = str(mr.config.visual_stimulus_parameters.spatial_frequency).replace(
            ".", "p"
        )
        gain = str(signal_gain)
