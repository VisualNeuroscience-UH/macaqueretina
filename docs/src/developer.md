...
You can add new parameters to existing yaml files at the root level, which become Configuration objects under macaquretina.config as such. In case you wish for validation of such new parameters, ie when somebody else will be using the software, you need to build it into the parameter_validation.py in pydantic framework.
