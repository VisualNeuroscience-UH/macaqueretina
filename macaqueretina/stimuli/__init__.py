...
def make_stimulus(options=None):
    ...
    if current_config is None:
        print("Configuration not found. Run mr.load_parameters() before accessing mr.make_stimulus_video.")
        return []
    
    current_hash = current_config.hash()
    if _cached_stimulus_instance is None or _cached_stimulus_instance[0] != current_hash:
