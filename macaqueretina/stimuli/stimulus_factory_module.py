...
class StimulusPattern:
    ...
    def iterating_natural_images(self):
        ...
        # dtype conversion for getting rid of a flaky error in assignment.
        ...
        if dtype == 'float16':
            self.frames = self.frames.astype(np.float16)
        elif dtype == 'uint8':
...
class StimulusFactory(VideoClass):
    """
    ...
    def generate(self, options: dict | None = None) -> np.ndarray:
        """
        ...
        Returns
        -------
         stimulus_video: StimulusFactory
